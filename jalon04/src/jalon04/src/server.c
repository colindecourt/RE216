    ...
    else
    {
      ...
      if (fds[i].revents == POLLIN)
      {
        ...
        else if (strncmp(buffer, "/msg", 4) == 0)
        {
          ...
          curUser = searchUser(UserTable, i, nb_clients, curUser);
          char *curPseudo;
          curPseudo = curUser->pseudo;

          int sockClient;
          struct user_table * wanted_user = NULL;

          struct channel *temp_channel = NULL;
          temp_channel = channel_table;
          struct channel *curChannel = NULL;
          char *channelName = malloc(CHANNEL_LEN_MAX);

          while (temp_channel->next_channel != NULL){
            for (int i = 0 ; i<CAPACITY_CHANNEL ; i++){
              if (strncmp(temp_channel->connected_people[i], curPseudo, strlen(curPseudo) == 0)){
                strncpy(channelName, temp_channel->channel_name, strlen(temp_channel->channel_name));
                curChannel = temp_channel;
                break;
              }
            }
          }
          for (int i = 0 ; i<CAPACITY_CHANNEL ; i++){
            if (strncmp(temp_channel->connected_people[i] , curPseudo, strlen(curPseudo) != 0)){
              sockClient = search_user_pseudo(UserTable, temp_channel->connected_people[i], nb_clients, wanted_user);
              do_send(fds[sockClient].fd, msg_cli, strlen(msg_cli));
            }
          }
        }
      }
